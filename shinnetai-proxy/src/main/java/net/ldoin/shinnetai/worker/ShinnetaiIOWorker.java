...
public abstract class ShinnetaiIOWorker<S extends ShinnetaiStatistic> extends ShinnetaiWorkerContext<S> {
    ...
    protected ShinnetaiIOWorker(Logger logger, PacketRegistry registry, InputStream input, OutputStream out, S statistic, WorkerOptions options) {
        ...
        this.outQueue = new LinkedBlockingQueue<>(options.getMaxQueueSize());
    ...
    public synchronized void start() {
        ...
        if (options.isKeepAlive()) {
            Thread.ofVirtual().name("keep-alive-worker").start(this::keepAliveLoop);
        }
    }
    ...
    public void handlePacket(ReadPacket readPacket) throws IOException {
        ...
        if (getSide() == PacketSide.SERVER && !handshaked) {
        ...
        if (packet instanceof PingPacket ping) {
            sendPacket(new PongPacket(ping.getTimestamp()));
            return;
        }
    ...
    private void readerLoop() {
        try {
            while (running.get()) {
                int length;
                try {
                    length = IOUtil.readVarInt(input);
                } catch (IOException e) {
                    break;
                }

                if (length > options.getMaxPacketSize()) {
                    getLogger().log(Level.SEVERE, "Packet too large: " + length + " > " + options.getMaxPacketSize());
                    try {
                ...
                if (length > 10 * 1024 * 1024) {
                     ...
                     if (length > freeMemory / 2) {
                         getLogger().log(Level.SEVERE, "Packet too large for current heap: " + length);
                         close();
                         break;
                     }
                }
                ...
                try {
                    int bytesRead = IOUtil.readFully(input, packetData, length);
                    if (bytesRead < length) {
                        break;
                    }
                } catch (IOException e) {
                ...
                try {
                    readPacket = readPacket(packetData, 0, length);
                } catch (Exception e) {
                    continue;
                }
