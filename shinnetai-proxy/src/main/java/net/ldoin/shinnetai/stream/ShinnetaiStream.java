...
public abstract class ShinnetaiStream extends ShinnetaiWorkerContext<ShinnetaiStatistic> implements Runnable {
    ...
    protected ShinnetaiStream(UUID uuid, ShinnetaiIOWorker<?> worker, ShinnetaiStreamOptions options) {
        super(Logger.getLogger("Stream (" + uuid + ")"), worker.getRegistry(), worker.getInput(), worker.getOut(), worker.getResponseWaiter());
