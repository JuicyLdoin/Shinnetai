...
public class ShinnetaiOutStream extends ShinnetaiStream {
    ...
    private volatile boolean commitSent = false;
    private volatile CompletableFuture<StreamCommitResult> commitFuture;
    ...
    public ShinnetaiOutStream(int id, ShinnetaiIOWorker<?> worker, ShinnetaiStreamOptions options) {
        ...
        this.queue = new LinkedBlockingQueue<>(options.getMaxQueueSize());
    }
    ...
    public void run() {
        ...
        if (options.isCommitGuarantee() && !commitSent) {
            commit();
        }
    ...
    public CompletableFuture<StreamCommitResult> commit() {
        if (commitSent) {
            return commitFuture != null ? commitFuture : CompletableFuture.completedFuture(StreamCommitResult.failed(getId(), "Commit already sent"));
        }

        commitSent = true;
        commitFuture = new CompletableFuture<>();

        worker.registerStreamCommit(getId(), commitFuture);
        worker.addPacket(WrappedPacket.of(new StreamCommitPacket(getId())));

        long timeout = options.getCommitTimeoutMs();
        if (timeout > 0) {
            Thread.ofVirtual().start(() -> {
                try {
                    Thread.sleep(timeout);
                    if (commitFuture.complete(StreamCommitResult.timeout(getId()))) {
                        worker.unregisterStreamCommit(getId());
                    }
        ...
        if (options.getOnCommit() != null) {
            commitFuture.thenAccept(options.getOnCommit());
        }
