...
public class ShinnetaiServer<C extends ShinnetaiConnection<?>> implements Runnable {
    ...
    private void handleConnection(Socket clientSocket) {
        try {
            int handshakeTimeout = 5000;
            try {
                clientSocket.setSoTimeout(handshakeTimeout);
            ...
            int id;
            try {
                typeId = IOUtil.readVarInt(input);
                id = IOUtil.readVarInt(input);
            ...
            logger.log(Level.SEVERE, "Failed handling connection", e);
            try {
                clientSocket.close();
