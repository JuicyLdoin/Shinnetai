...
public class ShinnetaiNodeConnection extends ShinnetaiConnection<ShinnetaiCluster<?>> {
    ...
    public ShinnetaiNodeConnection(ShinnetaiCluster<?> server, int connectionId, PacketRegistry registry, ShinnetaiConnectionStatistic statistic, Socket socket, ShinnetaiRegisteredNode node, ConnectionType connectionType) throws IOException {
        super(server, connectionId, registry, statistic, socket, Logger.getLogger("Cluster Node[" + node.getGroup() + "] (" + node.getAddress() + ":" + node.getPort() + ")"));
        this.node = node;
