...
public class IdGenerator {
    ...
    public synchronized int getNextId() {
        ...
        if (id == null) {
            ...
            if (id < 0) {
                throw new IllegalStateException("ID generator exhausted: integer overflow detected. " +
                        "Too many IDs have been allocated without being released.");
            }
        }
    ...
    public synchronized boolean reserve(int id) {
        ensureCapacity(id);
        if (id >= nextId.get()) {
            nextId.set(id + 1);
        }

        return markAllocated(id);
    }
