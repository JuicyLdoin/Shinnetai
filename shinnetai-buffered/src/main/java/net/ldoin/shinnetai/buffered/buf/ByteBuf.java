...
public class ByteBuf implements Buffered, Cloneable {
    ...
    public byte[] readBytes(int amount) {
        if (amount < 0) {
            throw new IndexOutOfBoundsException("Cannot read negative number of bytes: " + amount);
        }

        checkReadable(amount);
        byte[] result = new byte[amount];
        System.arraycopy(bytes, readIndex, result, 0, amount);
        readIndex += amount;
        return result;
    }
    ...
    protected void expand(int additional) {
        ...
        if (needed < 0) {
            throw new OutOfMemoryError("ByteBuf overflow: required capacity exceeds Integer.MAX_VALUE");
        }
    ...
    protected void expand(byte[] target) {
        int needed = writeIndex + target.length;
        if (needed > bytes.length) {
            int newCapacity = bytes.length == 0 ? Math.max(16, target.length) : bytes.length;
            while (newCapacity < needed) {
                int doubled = newCapacity << 1;
                if (doubled <= 0) {
                    newCapacity = needed;
        ...
        System.arraycopy(target, 0, bytes, writeIndex, target.length);
        writeIndex += target.length;
    }
